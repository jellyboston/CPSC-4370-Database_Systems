...
class PrecedenceGraph:
    ...
    def _has_conflict(self, operation1: Operation, operation2: Operation) -> bool:
        ...
        write = operation1.type == "write" or operation2.type == "write"

        # 2. different transactions
        diff_trans = operation1.transaction != operation2.transaction

        # 3. same item (e.g. A and A)
        same_item = operation1.item == operation2.item

        return write and diff_trans and same_item
    ...
    def _build_graph(self, schedule: Schedule):
        ...
        flat = [op for row in schedule for op in row if op is not None]
        ...
        # ~8 lines of code
        # check for all conflict pairs O(n^2)
        for i in range(len(flat)):
            for j in range(i+1, len(flat)):
                op1, op2 = flat[i], flat[j]
                if self._has_conflict(op1, op2):
                    src, dst = self.nodes[op1.transaction], self.nodes[op2.transaction]
                    src.add_edge(dst)
